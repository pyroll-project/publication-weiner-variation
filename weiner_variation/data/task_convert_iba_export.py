...
from weiner_variation.data.config import IBA_EXPORT_FILES, RAW_DATA_FILES

for in_file, out_file in zip(IBA_EXPORT_FILES, RAW_DATA_FILES):
    ...
    def task_convert_iba_export(depends_on: Path, produces: Path):
        df_in = pd.read_csv(depends_on, header=0, index_col=0, skiprows=[0, 2])

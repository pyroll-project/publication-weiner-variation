...
@pytask.mark.task()
@pytask.mark.depends_on(DATA_DIR / "raw_data" / "2022-09-26_01.csv")
...
def task_plot_timeline_pass_finding(depends_on: Path, produces: dict[..., Path]):
    ...
    raw_data: pd.DataFrame = pd.read_csv(depends_on, header=0, index_col=0)
    ...
    ax.grid(True)
    ax.legend()
    ax.set_xlabel(r"Time in $\mathrm{s}$")
    ax.set_ylabel(r"Roll Torque in $\mathrm{kNm}$")

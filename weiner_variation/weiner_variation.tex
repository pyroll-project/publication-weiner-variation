...
\usepackage[sorting=anyt, maxbibnames=99, maxcitenames=6, backend=biber, style=authoryear]{biblatex}
...
\extratitle{%
    ...
    Institute for Metal Forming, TU Bergakademie Freiberg
    ...
    Writing - Review \& Editing, Data curation, Software\\
    Institute for Metal Forming, TU Bergakademie Freiberg
    ...
    \minisec{Funding}
    PyRolL is currently supported by the Sächsische Aufbaubank (SAB) and the European Regional Development Fund (ERDF) program under the grant number 100688987.

    \minisec{Data Availability}
    Data openly available in a public repository that does issue DOIs.
    \url{https://doi.org/10.5281/zenodo.13960836}

    \minisec{Conflicts of Interest}
    The authors declare no conflicts of interest.

    \minisec{Keywords}
    Rolling Simulation; Open Source; Groove Rolling; Process Uncertainty; Monte Carlo (MC)
}
...
\begin{abstract}
    ...
    In this work, a Monte-Carlo-Method is applied to a fast hot rolling model based on the open-source rolling simulation framework PyRolL for estimating and analyzing the variational behavior of an experimental semi-continuous rolling line for steel wire production.
...
The method was found to give a good estimate of a rolling process' variational behavior, however, dependent on the included influences and availability of statistical data that describe the input parameters.
...
Inclusion of static control behavior is possible and required for industrial usage, as otherwise the deviation would be heavily overestimated.
Dynamic behavior can not be regarded with the current setup, since the used simulation procedure is stationary.
